...
class HappyMove(Node):  # 簡単な移動クラス
    ...
    def rotate_angle(self, angle):  # 指定角度を回転する
      error = 0.05  # 許容誤差を少し緩める
      diff = angle - (self.yaw - self.yaw0)  # 目標角度との差分
      diff = math.atan2(math.sin(diff), math.cos(diff))  # -π~π に正規化
  
      if math.fabs(diff) > error:  # 許容誤差を超える場合のみ回転
          self.get_logger().info(f'Yaw: {self.yaw:.2f}, Target: {angle:.2f}, Diff: {diff:.2f}')
          
          # 誤差が大きいときは速く、小さいときは遅くする
          angular_speed = max(0.05, min(0.3, 0.5 * math.fabs(diff)))  # 0.05 ~ 0.3 rad/s
          angular_speed *= 1 if diff > 0 else -1  # 回転方向を決定
